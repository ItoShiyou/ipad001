...
| FR-10 | 単ページ / 見開き | `Performance/PerformanceViewModel.swift`, `PageImageView.swift`, `ScorePagingView.swift` | ✅ 2026-09-05、依頼により**手動切替を廃止し、画面の向きから自動的に決めるよう変更**（縦置き＝単ページ・縦スクロール、横置き＝見開き2ページ・横スクロール）。要件の文言は「切り替えられる」だが、切替の手段が「トグルボタン」から「向きを変える」に変わったと解釈している |
...
| NFR-03 | 3ページのみ保持 | `Rendering/PageCache.swift` | ⚠️ 単ページ表示では3枚。**見開き表示では最大6枚**（現在の見開き2枚＋前後1見開きぶんずつ）。2026-09-05、ページ送りをスクロール操作にしたことで戻る方向のドラッグも前進と同じ頻度で起こるようになったため、以前の非対称な窓（前1枚・次2枚、最大5枚）から前後対称に広げた。`PageCache` 自体は上限を持たず、`PerformanceViewModel.refreshWindow` が渡す窓の大きさがそのまま保持数になる。要件文言（3ページのみ）と食い違うため、見開き時の挙動として妥当と判断するなら要件側の文言を直すべき |
...
| （新規要望）ページ送りのスクロール化 | 譜面を指でスクロールしてめくれるようにする。縦置き＝縦スクロール・単ページ、横置き＝横スクロール・見開き2ページ | `Performance/ScorePagingView.swift` | ✅ ベータ。SwiftUI の `ScrollView` + `.scrollTargetBehavior(.paging)` を使い、常に「前・現在・次」の3枠だけを並べて中央へ無音で付け替える循環スクロール方式（`PrerenderCoordinator`/`PageCache` には手を入れていない）。ドラッグ中は注釈を薄く隠す（追従しないため）。コントロールバー開閉スワイプは、ページ送りに使っていない方の軸（縦置きなら横方向、横置きなら縦方向）に割り当て直した。**実機でのドラッグの感触・ちらつき・軸の衝突は未検証** |
| （新規要望）自動スクロール | 速度を指定して自動でページを送る | `Performance/PerformanceViewModel.swift`（`autoScrollSecondsPerPage`/`toggleAutoScroll`）、`PerformanceView.swift` の再生ボタンと速度スライダー | ✅ ベータ。要件定義 FR-90〜99（演奏タイミングを記録して再生する本格機能）とは別物の、単純な一定間隔の自動ページ送り。3〜30秒/ページで調整可、曲末で自動停止、タップ・ペダル・スクラバー・ジャンプなど人間の操作が来たら即座に停止する（設計原則 P6）。下記「保留・v1対象外」に記録していた「自動でページをめくる機能は実機検証なしに着手しない」という判断を、依頼により今回は部分的に踏み越えている（記録のみ改める簡易版とし、FR-90〜99のフル実装は引き続き見送り）。**実機での動作は未検証** |
...
| 拡大・スクロール位置の記憶（FR-15） | 未着手。2026-09-05にページ送り用のスクロール（`ScorePagingView`）は入ったが、これはページ単位で位置が確定する「めくり」の代替であり、FR-15 が指すページ内の**任意の拡大・パン位置**の記憶とは別物。ピンチズームの導線自体がまだ無い |
| Android 版 | P4 |
