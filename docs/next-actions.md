...
> **2026-09-05 追記**: 実機（iPad Air 第5世代）が繋がったセッションで、Claude 自身が
> `xcrun xctrace` で os_signpost を計測した。「めくり操作→表示中ページの状態更新」だけなら
> 中央値約21ms（46回中30回が16ms超）、原因（actor越しのawait）を1つ潰して大幅改善した。
> ただし**これは glass-to-glass（実際に画面が塗り替わる瞬間まで）の計測ではない**。
> 詳細と残課題は [`implementation-status.md`](implementation-status.md) の
> 「NFR-01実機計測」を参照。以下の手順は、真の glass-to-glass 計測（高速度カメラでの
> フレームカウント）がまだ必要という前提でそのまま有効。
